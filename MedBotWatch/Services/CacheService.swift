...
class CacheService: ObservableObject {
    ...
    private let lastMedicationResponseKey = "cache_last_medication"
    ...
    // MARK: - Medication Cache

    func cacheMedicationResponse(_ response: MedicationResponse, forQuery query: String) {
        let cacheData = CachedResponse(query: query, response: response, timestamp: Date())
        if let encoded = try? JSONEncoder().encode(cacheData) {
            userDefaults.set(encoded, forKey: lastMedicationResponseKey)
        }
    }

    func getCachedMedicationResponse(forQuery query: String) -> MedicationResponse? {
        guard let data = userDefaults.data(forKey: lastMedicationResponseKey),
              let cached = try? JSONDecoder().decode(CachedResponse<MedicationResponse>.self, from: data) else {
            return nil
        }
        ...
        // Exact match for medication queries (drug names should match)
        if cached.query.lowercased().contains(query.lowercased()) ||
           query.lowercased().contains(cached.query.lowercased()) {
            return cached.response
        }
    ...
    func clearAllCache() {
        ...
        userDefaults.removeObject(forKey: lastMedicationResponseKey)
