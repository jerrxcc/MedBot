...
class SymptomViewModel: ObservableObject {
    ...
    func analyzeSymptoms(language: String = "auto") async {
        ...
        if let cached = cacheService.getCachedSymptomResponse(forQuery: query) {
            response = cached
            isLoading = false
            // Auto-speak cached summary
            await TTSService.shared.speak(text: cached.summary.short)
            return
        }
        ...
        do {
            ...
            // Auto-speak the short summary
            await TTSService.shared.speak(text: result.summary.short)
